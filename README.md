# DS18B20 for ESP32 Series (Optimized for C3)
...
4. **ESP32-C3 高速・高精度レジスタ制御**:
   * Arduino 標準 API (`digitalWrite`, `digitalRead`, `pinMode` 等) を一切使用せず、ESP32-C3 の GPIO レジスタを直接操作することで、関数の実行オーバーヘッドを極限まで削減しました。1-Wire プロトコルが要求するシビアなタイムスロット（15us以内のサンプリング等）を、FreeRTOS クリティカルセクション保護下で極めて精密に制御します。

## 互換性と制限事項
本ライブラリは ESP32 シリーズ全般で動作するように設計されていますが、以下の制限事項があります。

* **対応チップ**: ESP32, ESP32-S2, ESP32-S3, ESP32-C3 (RISC-V)
* **GPIO ピン番号の制限**:
  * **GPIO 0 〜 31** の範囲のピンのみ使用可能です。
  * 元祖 ESP32 (Dual-core) 等にある **GPIO 32 〜 39** のピンには対応していません。これは、本ライブラリが高速化のために 32ビット幅の単一レジスタバンク（GPIO 0-31用）を直接操作しているためです。
* **C3 最適化**: 
  * 特にシングルコアである ESP32-C3 において、Wi-Fi 通信等のバックグラウンド処理によるタイミング崩れを防止するために最適化されています。
